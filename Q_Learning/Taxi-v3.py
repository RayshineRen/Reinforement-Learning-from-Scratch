...
def update_Q_table(state, action, reward, state_prime, alpha, gamma):
    qsa = np.max(Q_table[state_prime])
    Q_table[state][action] += alpha * (reward + gamma * qsa - Q_table[state][action])
...
for episode in range(episodes):
    ...
    while True:
        ...
        update_Q_table(state, action, reward, state_prime, alpha, gamma)
        state = state_prime
        r += reward
        if done:
            break
